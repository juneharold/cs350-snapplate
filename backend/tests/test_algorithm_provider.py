...
def test_build_profile_provider_defaults_to_deterministic(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from app.config.lifespan import _build_profile_provider
    from app.services.algorithm.providers import DeterministicProvider

    monkeypatch.delenv("ALGORITHM_PROVIDER", raising=False)
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
...
def test_build_profile_provider_rejects_openai_without_api_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from app.config.lifespan import _build_profile_provider
